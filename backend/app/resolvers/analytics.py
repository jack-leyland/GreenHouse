...
def create_analytics(local_df):
    ...
    analytics.mean_current_environment_impact = local_df["environment-impact-current"].mean()
    analytics.mean_current_energy_consumption = local_df["energy-consumption-current"].mean()
    ...
    analytics.mean_potential_energy_efficiency = local_df["potential-energy-efficiency"].mean()
    analytics.mean_potential_energy_rating = convert_from_SAP(
        analytics.mean_potential_energy_efficiency
    )
    analytics.mean_potential_environment_impact = local_df["environment-impact-potential"].mean()
    analytics.mean_potential_energy_consumption = local_df["energy-consumption-potential"].mean()
    analytics.mean_potential_co2_consumption = local_df["co2-emissions-potential"].mean()
    ...
    analytics.mean_potential_hot_water_cost = local_df["hot-water-cost-potential"].mean()
    analytics.normalised_current_energy_efficiency = local_df["current-energy-efficiency-per-total-floor-area"]
    analytics.normalised_current_environment_impact = local_df["environment-impact-current-per-total-floor-area"]
    analytics.normalised_current_energy_consumption = local_df["energy-consumption-current-per-total-floor-area"]
    analytics.normalised_current_co2_consumption = local_df["current-energy-efficiency-per-total-floor-area"]
    analytics.normalised_current_lighting_cost = local_df["lighting-cost-current-per-total-floor-area"]
    analytics.normalised_current_heating_cost = local_df["heating-cost-current-per-total-floor-area"]
    analytics.normalised_current_hot_water_cost = local_df["hot-water-cost-current-per-total-floor-area"]
