...
def convert_to_rating(dataframe, house_rating_metrics):
    for house_rating in house_rating_metrics:
        # #DELETE THIS FOR NON BIGQUERY
        # fixed_metric = house_rating.upper().replace("-", "_")
...
def av_annual_rating_data(dataframe, features=timeseries_metrics):
    ...
    for i, house in dataframe.iterrows():
        ...
        if i + 1 < len(dataframe):
            other_house_address = dataframe.iloc[
                i + 1, dataframe.columns.get_loc("ADDRESS")
            ]
            if other_house_address == address:
                year_diff_neg = int(
                    pd.to_datetime(house["INSPECTION_DATE"]).year
                ) - int(
                    pd.to_datetime(
                        dataframe.iloc[
                            i + 1, dataframe.columns.get_loc("INSPECTION_DATE")
                        ]
                    ).year
                )
                for feature_name in features:
                    ...
                    if year_diff_neg < 0:
                        difference = (
                            dataframe.iloc[
                                i + 1, dataframe.columns.get_loc(feature_name)
                            ]
                            - house[feature_name]
                        )
                        year_diff = abs(year_diff_neg)
                        if (
                            year_diff > 0 and difference > 0
                        ):  # EPC sometimes gives negative over time
                            diff = difference / year_diff
                            if diff > 0 or diff < 0:
                                feature_averages[feature_name] += diff
...
def timeseries_data(dataframe, features=timeseries_metrics, years=L10_years):
    ...
    result = {}
    # Create new column in dataframe with "years"
