...
export default function EpcChart({ data, analytics, setModalHandler }: props) {
  ...
  const labelStyling =
    " pr-1 shadow-sm animate-widthGrow text-right font-bold text-white";
  ...
  return (
    <div className="flex h-5/6">
      ...
      <div className="w-2/6 h-full border border-r-0 border-y-0 px-4">
        ...
        <div className="pt-4 italic text-sm">Your House</div>
        <div className="border border-x-0 border-t-0">
          Rating: <b>{data.currentEnergyRating}</b> Score:{" "}
          <b>{data.currentEnergyEfficiency}</b>
        </div>
        <div className="pt-4 italic text-sm">Area Average</div>
        <div className="border border-x-0 border-t-0">
          Rating: <b>{analytics.meanCurrentEnergyRating}</b> Score:{" "}
          <b>{analytics.meanCurrentEnergyEfficiency}</b>
        </div>
        <div className="pt-4 italic text-sm">U.K. Average:</div>
        <div className="">
          Rating: <b className={`text-${epcIndexDictionary["D"]}`}>D</b> Score:{" "}
